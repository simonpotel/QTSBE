...
async def fetch_and_send_data(message, data_file, strategy):
    ...
    """
    reply_msg = [None, f"Analyse: {data_file}, {
        strategy}", discord.Color.green()]
    ...
    try:
        ...
        await reply_embed(message, "❌ Error", f"An unexpected error occurred: {e}\n\nTraceback:\n{tb_str}", discord.Color.brand_red())
...
async def save_json_and_image(json_data, data_file, strategy):
    # here we create temp files to not save anything since its will be save on discord
    temp_files = {}
    ...
    # generate plot figure (chart), with indicators, etc
    fig = generate_plot_figure(json_data, data_file, strategy)
    # spot where the code is saved (may we find a better a spot later)
    os.makedirs('display/python/saved_results/', exist_ok=True)
    # name file
    plot_html_filename = f'display/python/saved_results/{
        data_file}_{strategy}.html'
    # write the html code generated by plotly
    fig.write_html(plot_html_filename)
    temp_files['html'] = plot_html_filename  # temp file here

    # generate the image of the HTML plot in a temp file
    with tempfile.NamedTemporaryFile(delete=False, suffix='.png', mode='wb') as temp_image:
        pio.write_image(fig, temp_image.name)
        # stock the temp file name of the html plot img
...
async def send_data_to_discord(message, json_data, temp_files, data_file, strategy):
    drawdown_stats = json_data.get('stats', {}).get(
        'drawdown:', {})  # data of drawdown from api request
    positions_stats = json_data.get('stats', {}).get(
        'positions', {})  # data of positions from api request
    current_position_stat = json_data.get('result', [])[2]

    embed = discord.Embed()

    # file objects discord for attachements
    file_json = discord.File(temp_files['json'], filename=f"{
                             data_file}_{strategy}.json")
    file_html = discord.File(temp_files['html'], filename=f"{
                             data_file}_{strategy}.html")
    ...
    if isinstance(current_position_stat, list):
        for idx, position in enumerate(current_position_stat, 1):
            for key, value in position.items():
                embed.add_field(name=f"{key.replace('_', ' ').title()} {
                                idx}", value=value, inline=False)
    else:
...
def generate_plot_figure(json_data, data_file, strategy):
    ...
    fig.add_trace(go.Candlestick(x=dates, open=opens, high=highs,
    ...
    if len(trade_ratios) > 0:
        ...
        cumulative_ratios = [float(
            cumulative_ratio) for cumulative_ratio in json_data["stats"]["positions"]["cumulative_ratios"]]
    ...
    ratios = [float(ratio)
              for ratio in json_data["stats"]["positions"]["all_ratios"]]

    # Hover texts for buy and sell markers
    buy_hover_texts = [f"Index: {index}<br>Price: {price}<br>Date: {date}<br>Buy Signal: {
        buy_signal}" for index, price, date, buy_signal in zip(buy_indices, buy_prices, buy_dates, buy_signals)]
    sell_hover_texts = [f"Index: {index}<br>Price: {price}<br>Date: {date}<br>Ratio: {ratio}<br>Sell Signal: {
        sell_signal}" for index, price, date, ratio, sell_signal in zip(sell_indices, sell_prices, sell_dates, ratios, sell_signals)]
    ...
    if 'RSI' in indicators or 'Normalize_MACD' in indicators:
        ...
        fig.add_shape(type="line", x0=min(dates), y0=50, x1=max(
            dates), y1=50, row=2, col=1, line=dict(color="LightSkyBlue", width=3))
