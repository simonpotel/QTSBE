...
def get_file_data(pair):
    file_path = f"data/{pair}.csv"  # path of the CSV file that contents data of the pair
    try:
        ...
        return []
    data = [[str(row["Date"]), str(row["Open"])] for row in csv_data] # transforming CSV data into a list of lists
